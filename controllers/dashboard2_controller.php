...
#filtered recent initiatives
	...
	{
#query for the cost savings total for the week
		...
		#query for getting the largest entry (for the week)
		$sql_largest = $con->prepare("SELECT m.teamName, h.techName, y.typeName, v.envName, c.csCause, c.csSteps, c.csSavings, c.csActor 
					FROM costsavings c 
...
#query for the monthly total (bar graph)
		for($i = 1; $i <= $valMonth; $i++)
		{
			#$monthName = date('F', mktime(null, null, null, $i, 1));
			#loop the sql statements for the monthly data
...
#query for the pie/doughnut charts
...
#query for the 10 latest initiative input
